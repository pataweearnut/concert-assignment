        ...
        <Layout>
            <div className="min-h-screen p-6 md:p-10">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-0 xl:gap-6 mb-8">
                    {SUMMARY_CARD_CONFIG
                        .map((item, index) => {
                            return <SummaryCard key={index} title={item.title} value={statistics?.[item.key] ?? 0} icon={item.icon} bgcolor={item.bgcolor} />
                        })}
                </div>
                <div className="flex gap-8 mb-8">
                    <button
                        onClick={() => setActiveTab('overview')}
                        className={`pb-2 px-3 text-[24px] cursor-pointer ${activeTab === 'overview'
                    ...
                    <button
                        ...
                        className={`pb-2 px-3 text-[24px] cursor-pointer ${activeTab === 'create'
                            ? 'text-[#1692EC] border-b-2 border-[#1692EC] font-semibold'
                            : 'text-[#5C5C5C] hover:text-[#1692EC]'
                            }`}
                    >
